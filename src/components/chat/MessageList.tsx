...
export function getToolLabel(toolName: string, args: Record<string, unknown>): string {
  ...
  if (toolName === "file_manager") {
    switch (args.command) {
      case "delete": {
        const f = getFilename(args.path as string | undefined);
        return f ? `Deleting ${f}` : "Deleting file";
      }
      case "rename": {
        const f = getFilename(args.path as string | undefined);
        return f ? `Renaming ${f}` : "Renaming file";
      }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation": {
                            ...
                            const label = getToolLabel(tool.toolName, (tool as any).args ?? {});
                            return (
