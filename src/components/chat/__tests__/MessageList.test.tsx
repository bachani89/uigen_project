...
import { MessageList, getToolLabel } from "../MessageList";
...
test("MessageList renders without crashing when no messages", () => {
...
test("getToolLabel: file_manager rename", () => {
  expect(getToolLabel("file_manager", { command: "rename", path: "/components/Old.jsx" }))
    .toBe("Renaming Old.jsx");
});

test("getToolLabel: file_manager list", () => {
  expect(getToolLabel("file_manager", { command: "list" }))
    .toBe("Listing files");
});
...
test("getToolLabel: unknown tool name is humanised", () => {
  expect(getToolLabel("some_other_tool", {}))
    .toBe("Some Other Tool");
});
